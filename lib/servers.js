...
export const getServersWithThreadCapacity = (ns, threadsRequired, script) => {
	...
	const capableservers = targets
		.map((server) => ({ server, maxRam: ns.getServerMaxRam(server) }))
		.filter(({ server, maxRam }) => Math.floor((maxRam - ns.getServerUsedRam(server)) / scriptSize) >= threadsRequired)

	if (capableservers.length) {
		// And return the smallest
		capableservers.sort((a, b) => a.maxRam - b.maxRam)
		servers.push(capableservers[0])
		threadsRequired = 0
	} else {
